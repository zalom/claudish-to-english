...
- **"drift"** - the output is a short list of candidate words the last rewrite dropped, with no command attached to it: some of them will be ordinary English, not real vocabulary. Show it verbatim inside a fenced code block. Never run `/claudish keep` on any of them yourself; only the user decides which ones, if any, are worth protecting.
